...
struct node *hardCodeNodes(struct node *tail) {
  tail = nodeInsertEnd(tail, 837);
  tail = nodeInsertEnd(tail, 99);
  tail = nodeInsertEnd(tail, -272);
  tail = nodeInsertEnd(tail, -46);
  tail = nodeInsertEnd(tail, 10);
  tail = nodeInsertEnd(tail, 347);
  tail = nodeInsertEnd(tail, 79);
  tail = nodeInsertEnd(tail, -187);
  tail = nodeInsertEnd(tail, 36);
  tail = nodeInsertEnd(tail, 777);
  tail = nodeInsertEnd(tail, -4);

  // only negative int
//  tail = nodeInsertEnd(tail, -272);
//  tail = nodeInsertEnd(tail, -2);
//  tail = nodeInsertEnd(tail, -367);
//  tail = nodeInsertEnd(tail, -89);
...
struct node* radixSortLinkedList(struct node *tail) {
  if (tail == NULL) {
    return NULL;
  }
...
struct node *radixSort(struct node *tail) {
  ...
  struct node* negativeTail = NULL;
  struct node* positiveTail = NULL;
  struct node* temp = tail;
  int flagCompletion = 0;

  while (!flagCompletion) {
    if (temp->n < 0) {
      negativeTail = nodeInsertEnd(negativeTail, -1 * temp->n);
    } else {
      positiveTail = nodeInsertEnd(positiveTail, temp->n);
    }

    temp = temp->link;
    if (temp == tail) { flagCompletion = 1; }
  }

  negativeTail = radixSortLinkedList(negativeTail);
  positiveTail = radixSortLinkedList(positiveTail);

  // reverse negativeTail
  // tail becomes head
  if (negativeTail != NULL) {
    struct node* prev = negativeTail;
    struct node* curr = negativeTail->link;
    ...
    // connect negative Nugget to Positive
    curr->link = positiveTail->link;
    positiveTail->link = prev;
  }
