...
// Inverse QFT — no cudaq::adjoint (MLIR cloneReversedLoop crash on variable bounds)
struct qft_inv {
...
// ── Ripple-carry ADD ──────────────────────────────────────────────────────────
// Register layout: reg_a[0..n-1], reg_b[0..n-1], carry[0..n] (n+1 qubits).
// Result lands in carry[0..n].  carry[n] is the overflow/sign bit.
struct add_kernel {
    __qpu__ void operator()(int a, int b, int n) {
...
// ── Ripple-carry SUB (two's complement add) ───────────────────────────────────
// Load a normally, load ~b into reg_b, set carry[0]=1 to complete -b = ~b+1.
struct sub_kernel {
    __qpu__ void operator()(int a, int b, int n) {
        cudaq::qvector reg_a(n);
        cudaq::qvector reg_b(n);
        ...
        for (int i = 0; i < n; ++i) {
            if ((a >> i) & 1)    x(reg_a[i]);
            if (!((b >> i) & 1)) x(reg_b[i]); // load ~b
        }
        x(carry[0]); // carry-in = 1 (two's complement)

        for (int i = 0; i < n; ++i) {
            x<cudaq::ctrl>(reg_a[i], reg_b[i], carry[i + 1]);
            x<cudaq::ctrl>(reg_a[i], reg_b[i]);
            x<cudaq::ctrl>(reg_b[i], carry[i], carry[i + 1]);
            x<cudaq::ctrl>(reg_b[i], carry[i]);
            x<cudaq::ctrl>(reg_a[i], reg_b[i]);
        }
        mz(carry);
    }
...
int64_t c2q_add(int64_t a, int64_t b) {
    ...
    auto r = cudaq::sample(add_kernel{}, (int)a, (int)b, n);
...
int64_t c2q_sub(int64_t a, int64_t b) {
    ...
    auto r = cudaq::sample(sub_kernel{}, (int)a, (int)b, n);
